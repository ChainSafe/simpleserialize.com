 ...
 */
function patchSszTypes<T extends Record<keyof T, Type<unknown>>>(
	sszTypes: T,
): T {
...
function replaceUintTypeWithUintBigintType<T extends Type<unknown>>(
	type: T,
): T {
	...
	if (type instanceof ListBasicType) {
		return new ListBasicType(
			replaceUintTypeWithUintBigintType(type.elementType),
			type.limit,
		) as unknown as T;
	}
	if (type instanceof VectorBasicType) {
		return new VectorBasicType(
			replaceUintTypeWithUintBigintType(type.elementType),
			type.length,
		) as unknown as T;
	}
	...
	if (type instanceof VectorCompositeType) {
		return new VectorCompositeType(
			replaceUintTypeWithUintBigintType(type.elementType),
			type.length,
		) as unknown as T;
	}
